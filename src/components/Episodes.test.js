...
const mockEpisodesData = {episodes: [
    {
      id: 553946,
...
test("render Episodes component and checking for the image", () => {
   const { rerender, queryAllByTestId } = render(<Episodes episodes={[]}/>)

   expect(queryAllByTestId(/episode/i)).toHaveLength(0);

   rerender(<Episodes episodes={mockEpisodesData.episodes}/>)

   expect(queryAllByTestId(/episode/i)).toHaveLength(5);
